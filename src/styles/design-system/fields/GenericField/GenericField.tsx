...
export default function GenericField(props: GenericFieldProps): JSX.Element {
  ...
  return (
    <div 
      className={`${styles.field} ${isBooleanReadOnly ? styles.fieldContainer : ''} ${className || ''}`}
    >
      {!hideLabel && (
      ...
      {edit ? (
        ...
        if (model.type === 'boolean') {
          return (
            <div className={styles.booleanReadOnlyContainer}>
              <input
                type="checkbox"
                checked={!!model.value}
                disabled
                aria-label={model.label}
                className={`${styles.checkbox} ${styles.booleanCheckbox}`}
              />
            </div>
          );
        }
        ...
        const valueContent = formattedValue ? formattedValue : <i className={styles.emptyValue}>לא הוזן</i>;
